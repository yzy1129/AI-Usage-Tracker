...
| GitHub Copilot | 部分支持 | 不支持 | 不支持 | 支持 | VS Code Chat 会话元数据 |
...
Codex 的数据会从 `~/.codex/sessions` 读取，并在会话文件更新时自动刷新。若历史文件中包含 `cwd` 工作目录信息，插件会优先筛选当前工作区相关会话。
